...
class ApiController extends Controller
{
	public function actionEveryDayCase(){
		$user_list = UserList::find()->asArray()->all();
		$db = yii::$app->db;
		foreach($user_list as $uval){
			$dbname = 'cowbtool_'.$uval['code'];
			$db->createCommand("use `$dbname`;")->execute();
			$admin_user = Adminsheet::find()->where(['<','job',2])->asArray()->all();
